  ...
  if (futureConflicts.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 animate-fadeIn">
        ...
        <div className="text-base font-display font-semibold text-primary mb-1">All clear!</div>
        <div className="text-sm text-secondary text-center">No clashes in the next 2 weeks</div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-1">
        <span className="text-base font-bold text-primary">Upcoming Conflicts</span>
        <span className="text-xs text-secondary bg-red-50 text-danger font-semibold px-2 py-0.5 rounded-full">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[11px] font-semibold text-secondary mb-1">
                  ...
                  {overlap && (
                    <span className="ml-2 primary inline-flex items-center gap-1">
                      <Clock size={9} /> {overlap}
                    </span>
                  )}
                </div>
                <div className="font-semibold text-sm text-primary leading-snug">
                  {c.item_a.title}
                  <span className="text-secondary font-normal mx-1.5">
                    {c.severity === 'minor' ? 'close to' : 'vs'}
                  </span>
                  {c.item_b.title}
                </div>
                <div className="text-[11px] text-secondary mt-1">{mitigationIdea(c)}</div>
              </div>
