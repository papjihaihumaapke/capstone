...
// Mirror the same localStorage helpers used in ScheduleItemCard
...
export default function Analytics() {
  ...
  const { items, conflicts, user } = useAppContext();
  ...
  const stats = useMemo(() => {
    // Use same source of truth as ScheduleItemCard: DB flag OR localStorage
    const doneIds = getDoneIds(user?.id || '');
    const isDone = (i: ScheduleItem) => !!i.completed || doneIds.has(i.id);

    const todayItems     = items.filter(i => itemOccursOnDate(i, todayStr));
    const todayDone      = todayItems.filter(isDone);
    const todayRemaining = todayItems.filter(i => !isDone(i));
    ...
    const perDay = last7.map(dateStr => {
      ...
      return {
        ...
        done: dayItems.filter(isDone).length,
      };
    });
  ...
  }, [items, conflicts, user?.id, todayStr]);
        ...
        <div className="max-w-[960px] mx-auto px-4 lg:px-8 pt-6 lg:pt-10">
          ...
          <div className="bento-tile flex flex-col items-center py-16 gap-3 text-center">
            ...
            <p className="text-[13px] text-muted leading-relaxed max-w-[240px]">
              Add items and mark them done — your stats will appear here.
            </p>
              ...
              <div className="bento-tile">
                {stats.todayItems.length === 0 ? (
                  ...
                  <>
                      ...
                      <div>
                        ...
                        <p className="text-[13px] text-secondary mt-1">
                          {stats.todayDone.length === 0
                            ...
                            ? 'All done for today!'
                            : `${stats.todayRemaining.length} still to go`}
                        </p>
                      </div>
                    ...
                    <div className="h-2 rounded-full overflow-hidden" style={{ background: '#F2F2EF' }}>
                      <div className="h-full rounded-full transition-all"
                        style={{ width: `${todayPct}%`, background: todayPct === 100 ? '#1A7A4A' : '#1A1A1A' }} />
                    </div>
                    {stats.todayRemaining.length > 0 && (
              ...
              <div className="bento-tile">
                <div className="flex items-end justify-between mb-4">
                  <div>
                    ...
                    <p className="text-[12px] text-muted mt-0.5">{weekPct}% completion this week</p>
                  </div>
                </div>
                <div className="flex items-end gap-1.5 h-16">
                  {stats.perDay.map(d => {
                    const totalH = Math.round((d.total / barMax) * 100);
                    ...
                    return (
                      <div key={d.dateStr} className="flex-1 flex flex-col items-center gap-1">
                        <div className="w-full flex flex-col-reverse rounded-[4px] overflow-hidden" style={{ height: '44px', background: '#F2F2EF' }}>
                          {d.total > 0 && (
                            <div className="w-full rounded-[4px]" style={{ height: `${totalH}%`, background: '#DEDBD5', position: 'relative' }}>
                              {d.done > 0 && (
                                <div className="absolute bottom-0 left-0 right-0 rounded-[4px]"
                                  style={{ height: `${doneH}%`, background: isToday ? '#1A7A4A' : '#1A1A1A' }} />
                              )}
                            </div>
                          )}
                        </div>
                        <span className="text-[10px] font-medium" style={{ color: isToday ? '#1A1A1A' : '#A8A8A8' }}>
                          {d.label}
                        </span>
                      </div>
                    );
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: '#F2F2EF' }}>
                          <div className="h-full rounded-full"
                            style={{ width: `${barPct}%`, background: '#1A1A1A', transition: 'width 0.4s ease' }} />
                        </div>
                      </div>
                    </div>
                  );
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div className="bento-tile">
                    ...
                    {stats.unresolved > 0 && (
                      <button onClick={() => navigate('/home')} className="mt-2 text-[11px] font-medium text-orange underline">
                        Review now
                      </button>
                    )}
                  </div>
                  <div className="bento-tile">
                    ...
                    <p className="text-[11px] text-muted mt-1">{pct(stats.resolved, conflicts.length)}% rate</p>
                  </div>
                </div>

                {stats.topConflicted.length > 0 && (
                  <>
                    <p className="text-[11px] font-semibold text-muted uppercase tracking-widest mb-3">Most conflicted</p>
                    <div className="bento-tile">
