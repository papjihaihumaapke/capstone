...
import { Upload, FileUp, AlertTriangle, CheckCircle2, Trash2 } from 'lucide-react';
  ...
  const DropZone = (
    <div
      ...
      className={`bento-tile cursor-pointer transition-all text-center ${
        dragActive ? 'ring-2 ring-dark ring-offset-2'
        : file ? 'ring-2 ring-dark/30 ring-offset-1'
        : 'hover:shadow-tile-hover'
      }`}
    >
      <input ref={fileInputRef} type="file" accept=".ics,text/calendar"
      ...
      {file ? (
        ...
        <div className="animate-fadeIn py-2">
          ...
          <p className="text-[14px] font-medium text-dark">{dragActive ? 'Drop here' : 'Click or drag to upload'}</p>
  ...
  return (
    <div className="min-h-[100dvh] bg-appbg pb-24 lg:pb-10 animate-fadeIn">
      <div className="max-w-[960px] mx-auto px-4 lg:px-8 pt-6 lg:pt-10">
        ...
        {/* Header */}
        <h1 className="text-[20px] font-semibold text-dark mb-6">Import Calendar</h1>
          ...
          <div className="space-y-4">
            ...
            <div className="bento-tile">
              ...
              <p className="text-[13px] text-secondary mb-3 leading-relaxed">
                Connect your Google account to automatically sync events into Acminder.
              </p>
              <button onClick={() => signInWithGoogle()}
                className="w-full flex items-center justify-center gap-2 py-2.5 rounded-[10px] text-[13px] font-medium hover:opacity-90 active:scale-[0.98] transition-all"
          ...
          <div className="space-y-4">
            ...
            <div className="bento-tile">
              ...
              <ol className="space-y-0 list-none">
                {[
                  { name: 'Google Calendar', steps: 'Settings (gear icon) → Import & export → Export' },
                  { name: 'Apple Calendar',  steps: 'File menu → Export → Export…' },
                  { name: 'Outlook',         steps: 'File → Open & Export → Import/Export → Export to a file → iCalendar (.ics)' },
                ].map((g, i, arr) => (
                  <li key={g.name} className="text-[13px] text-secondary leading-relaxed py-2.5"
                    style={i < arr.length - 1 ? { borderBottom: '1px solid rgba(0,0,0,0.06)' } : {}}>
                    <strong className="text-dark font-medium">{g.name}:</strong> {g.steps}
                  </li>
                ))}
              </ol>
            </div>
            ...
            <div className="bento-tile">
              <p className="text-[11px] font-semibold text-muted uppercase tracking-widest mb-3">
                Manage imports{sessions.length > 0 ? ` (${sessions.length})` : ''}
              </p>
              {sessions.length === 0 ? (
                <p className="text-[13px] text-muted">No imports yet. Import a .ics file and it will appear here.</p>
              ) : (
                <div className="space-y-0">
                  {sessions.map((session, i) => (
                    <div key={session.id} className="flex items-center gap-3 py-2.5"
                      style={i < sessions.length - 1 ? { borderBottom: '1px solid rgba(0,0,0,0.06)' } : {}}>
                      <div className="w-8 h-8 rounded-[8px] bg-appbg flex items-center justify-center shrink-0">
                        <FileUp size={14} className="text-muted" />
                      </div>
